...
2. 실행 중이면 Condition으로 보호된 FIFO bounded queue 자리 하나를 선점한다.
3. 대기 자리도 찼으면 즉시 `HTTP 429`를 반환한다.
4. 대기 요청은 최대 15초만 permit을 기다린다.
5. 모델 호출 성공·예외와 무관하게 `finally`에서 permit을 반환한다.

반환된 permit은 Queue 선두에만 전달한다. 새 요청은 대기자가 한 건이라도 있으면 즉시 permit을
가져갈 수 없으므로, 지속적인 요청 유입에서도 먼저 기다린 요청이 timeout까지 밀리지 않는다.
