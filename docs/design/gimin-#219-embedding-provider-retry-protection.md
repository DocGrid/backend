...
3. HTTP 429의 유효한 `Retry-After`는 최대 지연 안에서 최소 재시도 지연으로 반영된다.
...
|---|---|---|---|
| Read/connect timeout | `EMBEDDING_PROVIDER_TIMEOUT` | `EMBEDDING_PROVIDER_TIMEOUT` | 가능 |
...
| HTTP 4xx, 단 429 제외 | `EMBEDDING_REQUEST_REJECTED` | `EMBEDDING_REQUEST_INVALID` | 금지 |
...
2. `jittered = base × [0.8, 1.2]`
3. `Retry-After`가 있으면 `max(jittered, Retry-After)`
4. 실패가 Circuit을 열었거나 Open 전 시작한 요청이 늦게 실패하면 남은 Open 시간을 함께 적용
5. 최종 결과를 `retry-max-delay`로 제한

`Retry-After`는 delta-seconds 형식만 수용한다. 음수·비정상 값은 무시하고, 외부 값으로 무제한 지연이
생기지 않게 서버 최대 지연으로 제한한다.
