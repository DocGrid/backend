...
- 동시성 2 요청 p99는 `7.71s`, 문서 p99는 `11.85s`, Peak RSS는 `2,375.73 MiB`였다.
- 기존 동일 Profile보다 요청 p99는 5.1%, 문서 p99는 12.6%, Peak RSS는 5.5% 감소했다.
- 동시성 4에서는 21건이 모두 의도한 HTTP 429로 거절됐고 다른 오류는 0건이었다.
- 429 거절 p95/p99는 `5.74/5.94ms`였고 Provider는 OOM·재시작 없이 생존했다.
...
| 총 재시도 | 0회 | 0회 | 유지 |
| 요청 p95 | 7.71s | 7.33s | 4.9% 감소 |
| 요청 p99 | 8.12s | 7.71s | 5.1% 감소 |
| 문서 전체 p99 | 13.56s | 11.85s | 12.6% 감소 |
| 동시성 2 Peak RSS | 2,513 MiB | 2,375.73 MiB | 137.27 MiB, 5.5% 감소 |
| 실제 PDF OOM | 0회 | 0회 | 유지 |
| Provider 처리량 | 1.22 chunks/s | 1.25 chunks/s | 2.4% 증가 |
| v1→v3 전체 처리시간 | 21.68s | 20.76s | 4.2% 감소 |
| E2E 처리량 | 0.92 chunks/s | 0.963 chunks/s | 4.7% 증가 |
...
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 4 | 1 | 54/54 | 27/27 | 1.20 chunks/s | 4.09s | 5.00s | 7.59s | 2,275.70 MiB | 0 |
| 4 | 2 | 54/54 | 27/27 | 1.25 chunks/s | 7.33s | 7.71s | 11.85s | 2,375.73 MiB | 0 |
...
| HTTP 429 | 21건 |
| 예상 밖 오류 | 0건 |
| 429 p50 / p95 / p99 / max | 2.90 / 5.74 / 5.94 / 5.98ms |
| 성공 요청 p95 / p99 | 7.36 / 7.46s |
| 완료 처리량 | 1.25 chunks/s |
| Peak RSS | 2,361.20 MiB |
| OOM | 0회 |
| Container 생존 | 성공 |
| Container 재시작 | 0회 |
...
|---:|---:|---:|---:|---:|---:|---|
| v1 | 8 / 8 | 7.21s | 8.43s | 1 | 0 | 성공 |
| v2 | 6 / 6 | 4.54s | 5.96s | 1 | 0 | 성공 |
| v3 | 6 / 6 | 4.70s | 6.38s | 1 | 0 | 성공 |

- 전체 처리시간: `20.76s`
- 전체 처리량: `0.963 chunks/s`
- Job 성공률: `3/3 (100%)`
- 재시도: `0회`
