  ...
  return <section className="content page-view wide-page">
    ...
    <div className="users-layout">
      <div className="panel-card user-list">
        <form className="toolbar" onSubmit={search}><label className="toolbar-search"><span>⌕</span><input aria-label="사용자 검색" value={keywordInput} onChange={(event) => setKeywordInput(event.target.value)} placeholder="이름 또는 이메일 검색" /></label><select aria-label="사용자 부서" value={departmentId} onChange={(event) => { setDepartmentId(event.target.value); setPage(0); }}><option value="">부서 전체</option>{departments.map((department) => <option key={department.id} value={department.id}>{department.name}</option>)}</select><select aria-label="사용자 상태" value={status} onChange={(event) => { setStatus(event.target.value); setPage(0); }}><option value="">상태 전체</option>{["ACTIVE", "INACTIVE", "LOCKED", "DELETED"].map((item) => <option key={item}>{item}</option>)}</select><span className="toolbar-count">{data?.totalElements ?? 0}명</span><button type="button" onClick={resetFilters}>초기화</button><button className="filter-submit">검색</button></form>
